...
The project's reason to exist (`BRIEF.md` §5) is that generate-then-review beats hand-authoring.
That is falsifiable, so the first thing built is the smallest loop that produces the evidence:
**paste ~two pages of Japanese → get ~30 reviewed cards → study them on two consecutive days.**
...
- **Japanese, generation and review only, no scheduling.** Rejected: it deletes the half that shows
