...
- Generated by the Better Auth CLI, then they land in Drizzle's migration flow like everything
  else. **Phase 6 ran it, on 2026-09-09, in #4** — the output is `server/db/schema/auth.ts`.
  ⚠️ **Amended 2026-09-09:** this line used to give the command as
  `npx auth@latest generate --adapter drizzle --dialect pg`, and **those two flags produce the wrong
  output** — they make the CLI synthesise an adapter instead of reading the configured one, and the
  configured one is where `schemaName` lives, so the generator emits `pgTable(...)` in `public` and
  silently drops the `auth` schema. The working form is
  `npx auth@1.7.3 generate --config <config> --output server/db/schema/auth.ts`, with
  `drizzleAdapter(db, { provider: "pg", schemaName: "auth" })`. `04` §8 carried the same wrong
  command and is amended too.
