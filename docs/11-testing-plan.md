...
| ⚠️ A source of an undeclared kind | Added 2026-09-16 with #19. `anki` is in `04` §5.1's `CHECK` and in no pipeline, so a row carrying it is refused **by name** in both languages rather than running whichever pipeline is first — the *stage that silently does nothing* ADR 0063 rejected, one layer up |
