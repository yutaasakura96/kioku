...
| --- | --- | --- | --- | --- |
| **Unit** | `test/unit/` | plain Node, no Nuxt | none | Pure functions: the pipeline stages, the FSRS wrapper, the `from` allowlist, the grade validator, the metric arithmetic. ⚠️ **And §6.1's configuration assertions**, added 2026-09-09 — they read `nuxt.config` through `loadNuxtConfig()`, which needs neither a build nor a request, so the tier is "no Nuxt runtime" rather than "no file on disk". ⚠️ **And, since 2026-09-20 with [#26](https://github.com/yutaasakura96/kioku/issues/26), the `.apkg` reader** — which builds a SQLite file in `os.tmpdir()` with `node:sqlite` and reads it back. **"none" in this column has always meant *no Postgres, no container, no Docker*** (ADR 0038's property is that a laptop without Docker runs the whole TypeScript suite), and that is untouched: the fixtures are built and deleted inside the test, and the tier still runs in well under a second |
...
| ⚠️ **Whose reading** | Added 2026-09-13 with [#15](https://github.com/yutaasakura96/kioku/issues/15) and [ADR 0045](adr/0045-the-reading-half-of-the-identity-key-is-written-in-the-word-s-own-script.md) § Amended 2026-09-13. あります and ある are **one** *note*, `有る␟ある`, and 開いた and 開く are one, `開く␟ひらく` — the reading comes from `dictionary_form` re-tokenised whenever the surface inflected. **Three of these assert the rule and three assert what it must not break**: 六時's 時 still reads じ (uninflected, and トキ if re-read alone), an inflected する reads する and never なる (its `normalized_form` is 為る, which alone reads ナル), and サボった reads さぼる — the script rule from the row above, still running on the term. ⚠️ **The stage matters as much as the answer**: the re-tokenised reading is computed in stage 2, because stage 3 has no dictionary and §8's seam says it may not get one |
...
| A stage key is a module name | `03` §10. ⚠️ **All seven since #9**, which is the assertion this row said it was waiting for: `generate.py` and `write_pending.py` are stages 6 and 7's modules, and a stage with no module now fails by name. ⚠️ **Per *source kind* since #19** ([ADR 0063](adr/0063-the-input-is-a-chosen-word-list.md)): both declared pipelines are asserted whole, and a key nothing runs is a **startup failure** — `check_pipelines`, sabotaged to red in `test_pipeline.py` |
| ⚠️ A source of an undeclared kind | Added 2026-09-16 with #19. A row carrying a kind the declaration does not name is refused **by name** in both languages rather than running whichever pipeline is first — the *stage that silently does nothing* ADR 0063 rejected, one layer up. ⚠️ **Amended 2026-09-20 with [#26](https://github.com/yutaasakura96/kioku/issues/26)**: `anki` *was* that kind, and it is declared now ([ADR 0068](adr/0068-an-imported-deck-is-unpacked-by-the-app-into-a-word-list-that-feeds-generate.md)), so all three of `04` §5.1's values have a pipeline and the test reaches this case with a kind nothing declares |
| ⚠️ A deck's pipeline | Added 2026-09-20 with #26. `anki` is `word_list` with `unpack` in front, and two of its seven stages run in the **app** — `unpack` and `chunk`, both inside the submit transaction. They are declared anyway (ADR 0003) and named in `STAGES_RUN_ELSEWHERE`, so *a stage key with no module* stays a startup failure while these two stay legal. ⚠️ **And the deck's reading is carried, never keyed on**: a line whose deck reading disagrees with Sudachi's keys on Sudachi's (ADR 0068 §5) |
| ⚠️ A tabbed line | Added 2026-09-20 with #26. `normalise` reads the text before the first tab as the term **for every kind**: an `anki` line hands columns 2 and 3 to stage 6 as the deck's hints, and a `word_list` line discards anything after a tab. ⚠️ **A behaviour change for word lists** — such a line used to be kept whole as two content words |
| ⚠️ A word-list line | Added 2026-09-16 with #19. `normalise` over the real dictionary: an inflected entry gets the lemma's reading (あります → `有る␟ある`, ADR 0045), a katakana entry keeps katakana, a kana-only entry is read through its *normalized form* (ひらがな → `平仮名␟ひらがな`), and **an unresolvable line is kept** with an empty reading and `is_oov` — a loanword, a numeral (which `normalized_form` would rewrite to a digit) and a line holding two content words |
...
required and *judgement* and memory-bearing subsets, the *identity key*, the *template* keys, the
pipelines per *source kind*, `04` §5.1's `CHECK` values and the blank-character class — not the field
list alone, because each of them is derived by its own code on each side
