...
- **The backfill has not run.** `--estimate` put it at $0.17–$0.31 for 475 *notes* (§ Next).
...
- ~~[#28](https://github.com/yutaasakura96/kioku/issues/28) — the check is the *grade*, accepted
  ...
  **except the backfill's run on Neon**. `0006` is on Neon, and `--estimate` said (2026-09-21):
  **475 *notes*, 12 requests to `claude-sonnet-5`, 14,379 input tokens counted, $0.17–$0.31** (output
  assumed 30–60 tokens a *note*). The run waits on Yuta's approval of that figure. ⚠️ The backfill spends model money, and Yuta
  approves that run.
