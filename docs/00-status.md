...
- **Migration `0006`**, generated by drizzle-kit. ⚠️ **Not applied to Neon** — § Carrying.
- **The backfill has not run.** `--estimate` is written and free; its figure is the next thing
  Yuta sees, after `0006` is on Neon.
...
- ~~[#28](https://github.com/yutaasakura96/kioku/issues/28) — the check is the *grade*, accepted
  ...
  **except the backfill's run on Neon**, which waits on two things in order: `0006` applied to Neon,
  then Yuta's approval of the `--estimate` figure. ⚠️ The backfill spends model money, and Yuta
  approves that run.
...
- ⚠️ **Neon is at `0005` and the code expects `0006`** (2026-09-21, #28). `snapshotOf` joins
  `note_meaning` and `meaning_synonym`, so a deploy or a local `npm run dev` against Neon before
  `npm run db:migrate` fails every *Review* composition. The migration is two new tables and nothing
  else; applying it is the step before the backfill estimate.
